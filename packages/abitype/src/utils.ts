...
import { type Error, type Merge, type Pretty, type Tuple } from './types.js'
...
> = abiType extends SolidityBytes
  ? PrimitiveTypeLookup[abiType][abiParameterKind] // If `PrimitiveTypeLookup` is missing key values from AbiType, there will be an error on this property access.
...
export type BitsTypeLookup = {
  [_ in MBits]: ResolvedRegister[_ extends LessThanOrEqualTo48Bits
    ? 'IntType'
    : 'BigIntType']
}
...
> = abiParameter['type'] extends AbiBasicType
  ...
  : // 2. Check if type is tuple and convert each component
...
> = Pretty<{
  ...
  // Draft PR:
  // - https://github.com/microsoft/TypeScript/pull/55452
  // Tracking issues:
  // - https://github.com/microsoft/TypeScript/issues/56093
  // - https://github.com/microsoft/TypeScript/issues/44939
  [K in keyof abiParameters]: AbiParameterToPrimitiveType<
    abiParameters[K],
    abiParameterKind
  >
}>
 ...
 * @param stateMutability - {@link AbiStateMutability} to filter by
...
> = Extract<abi[number], { type: 'function'; stateMutability: stateMutability }>
...
export type ExtractAbiFunctionNames<
  abi extends Abi,
  stateMutability extends AbiStateMutability = AbiStateMutability,
> = ExtractAbiFunctions<abi, stateMutability>['name']
...
export type ExtractAbiFunction<
  ...
  stateMutability extends AbiStateMutability = AbiStateMutability,
> = Extract<ExtractAbiFunctions<abi, stateMutability>, { name: functionName }>
...
export type IsTypedData<typedData> = typedData extends TypedData
  ? {
      [key in keyof typedData]: {
        ...
        [key2 in
          typedData[key][number] as key2['type'] extends keyof typedData
            ? never
      ...
      [_ in keyof typedData]: Record<string, never>
    }
    ? true
    : false
  : false
...
> = {
  ...
  [key in
    keyof typedDataParameters]: typedDataParameters[key] extends infer typedDataParameter extends {
    name: string
    type: unknown
  }
    ? // 1. Check if type is struct
      ...
      typedDataParameter['type'] extends `${infer type extends keyof typedData &
          string}[${infer size}]`
      ? {
          name: typedDataParameter['name']
          type: `tuple[${size}]`
