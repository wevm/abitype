...
export type IsErrorSignature<type extends string> = type extends ErrorSignature<
  infer name
>
  ? IsName<name>
  : false
...
export type IsEventSignature<type extends string> = type extends EventSignature<
  infer name
>
  ? IsName<name>
  : false
...
export type IsStructSignature<type extends string> =
  type extends StructSignature<infer name> ? IsName<name> : false

type ConstructorSignature<tail extends string = string> = `constructor(${tail}`
export type IsConstructorSignature<type> = type extends ConstructorSignature
  ? type extends ValidConstructorSignatures
    ? true
    : false
  : false
...
export type FallbackSignature<stateMutability extends '' | ' payable' = '',> =
  `fallback() external${stateMutability}`
...
export type Signature<
  type extends string,
  position extends string | unknown = unknown,
> = IsSignature<type> extends true
  ? type
  : string extends type // if exactly `string` (not narrowed), then pass through as valid
  ? type
  : Error<`Signature "${type}" is invalid${position extends string
      ? ` at position ${position}`
      : ''}.`>

export type Signatures<type extends readonly string[]> = {
  [key in keyof type]: Signature<type[key], key>
}
...
export type IsValidCharacter<type extends string> =
  type extends `${ValidCharacters}${infer tail}`
    ? tail extends ''
      ? true
