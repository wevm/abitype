...
import { type Error, type Trim } from '../../types.js'
...
export type ParseStructs<signatures extends readonly string[]> =
  ...
  >
    ? // Resolve nested structs inside each struct
      {
        [key in keyof structs]: ResolveStructs<structs[key], structs>
      }
    : never
...
> = readonly [
  ...{
    [key in
      keyof abiParameters]: abiParameters[key]['type'] extends `${infer head extends string &
      keyof structs}[${infer size}]` // Struct arrays (e.g. `type: 'Struct[]'`, `type: 'Struct[10]'`, `type: 'Struct[][]'`)
...
export type ParseStructProperties<
  ...
  structs extends StructLookup | unknown = unknown,
  result extends any[] = [],
> = Trim<type> extends `${infer head};${infer tail}`
