...
test('ParseAbiParameter', () => {
  ...
  // assertType<ParseAbiParameter<'address calldata foo', OptionsWithModifier>>({
  //   type: 'address',
  //   name: ["Error: Invalid modifier. calldata not allowed in address type."],
  // })
  ...
  assertType<
    ParseAbiParameter<
      '((((string baz) bar)[1] foo) boo) calldata buu',
      OptionsWithModifier
    >
  >({
    type: 'tuple',
